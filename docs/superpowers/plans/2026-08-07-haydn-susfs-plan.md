...
- SUSFS uses `gki-android12-5.10` at `ee7dc7a03b7c836952cce55c5f3834de62a465d1`.
- The custom kernel patch is `patches/haydn-susfs-v2.2.0-5.4.patch`.
...
- [ ] **Step 1: Extend the haydn test.** Use its existing `config_value` helper and `fail` helper to assert exact values `KSU_VARIANT=sukisu-ultra`, `KSU_REF=builtin`, `ENABLE_SUSFS=true`, `SUSFS_BRANCH=gki-android12-5.10`, `SUSFS_REF=ee7dc7a03b7c836952cce55c5f3834de62a465d1`, and `SUSFS_KERNEL_PATCH=patches/haydn-susfs-v2.2.0-5.4.patch`.
- [ ] **Step 2: Create the helper test.** Build a temporary bare Git repository with a `kernel-5.4` branch and known commit, source `scripts/patches.sh` with a temporary `KERNEL_DIR`, and assert that `susfs_checkout_ref` makes `git rev-parse HEAD` equal the known full SHA. Set `SUSFS_KERNEL_PATCH=tests/haydn_build_config_test.sh` and assert `susfs_kernel_patch_for` returns `${ROOT}/tests/haydn_build_config_test.sh`; unset it and assert the generic return is `$patch_dir/50_add_susfs_in_kernel-5.4.patch`. Run an invalid SHA in a subshell and require nonzero status. The final profile path is verified separately by the haydn test.
...
**Files:** Create `patches/haydn-susfs-v2.2.0-5.4.patch`; verify official `Xiaomi_Kernel_OpenSource` at `haydn-r-oss`.

**Interfaces:** The patch consumes the upstream `gki-android12-5.10` patch at commit `ee7dc7a03b7c836952cce55c5f3834de62a465d1` and produces a strict `-p1` patch with no rejects or fuzzy hunks.
...
- [x] **Step 3: Generate `patches/haydn-susfs-v2.2.0-5.4.patch` with `git diff --no-ext-diff --binary`, then run `git apply --check --verbose patches/haydn-susfs-v2.2.0-5.4.patch` against a second clean haydn tree.** Expected: all hunks apply without error, fuzz, or `.rej` files.
- [x] **Step 4: Run the Action SUSFS stage against a clean temporary haydn tree with the pinned profile.** Verify logs contain the pinned commit, custom patch name, SUSFS version, and no rejects. Commit with `git add patches/haydn-susfs-v2.2.0-5.4.patch` and `git commit -m "Add haydn SUSFS kernel compatibility patch"`.
...
- [ ] **Step 1: Run `for test in tests/*_test.sh; do bash "$test" || exit $?; done`, `bash -n scripts/*.sh patches/*.sh tests/*_test.sh`, profile validation for `config.env config/*.env`, `git diff --check -- . ':!patches/*.patch'`, and `git apply --check patches/haydn-susfs-v2.2.0-5.4.patch` against clean haydn source.** All must exit zero. Generated patch files are checked by application because an outer whitespace scan misreads their inner context markers as added indentation.
...
- [ ] **Step 4: Require a successful build job and Image or AnyKernel3 artifact.** Resolve logs must show SukiSU-Ultra builtin, the pinned SUSFS commit, and SUSFS v2.2.0 with no patch, defconfig, or compile error. Record the workflow URL and final commit.
