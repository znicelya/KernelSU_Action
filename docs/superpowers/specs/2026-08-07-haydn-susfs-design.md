...
- SUSFS source: `simonpunk/susfs4ksu`, branch `kernel-5.4`, pinned to
  `76affd70abf61d77feb0a132f61365d6848505df` (SUSFS v1.5.5).
...
SUSFS_KERNEL_PATCH=patches/haydn-susfs-5.4.patch
```
...
The local `patches/haydn-susfs-5.4.patch` is derived from the pinned upstream
`50_add_susfs_in_kernel-5.4.patch`. It preserves the complete feature set while
adapting the conflicting hunks to Xiaomi's source:
...
SUSFS source files continue to come from the pinned upstream checkout.
